...
export default function Threads({
  ...
  useEffect(() => {
    ...
    function resize() {
      ...
      (program.uniforms as Record<string, { value: Color | Float32Array | number }>).iResolution.value = new Color(clientWidth, clientHeight, clientWidth / clientHeight);
    }
    ...
    function update(t: number) {
      if (enableMouseInteraction) {
        ...
        (program.uniforms as Record<string, { value: Float32Array }>).uMouse.value[0] = currentMouse[0];
        (program.uniforms as Record<string, { value: Float32Array }>).uMouse.value[1] = currentMouse[1];
      }
      (program.uniforms as Record<string, { value: number }>).iTime.value = t * 0.001;
