...
// Admin — TEAM MISSION CONTROL. One owner-only screen answering "what is my
// team doing, when, and what's stuck": the dispatch floor (board + decisions +
// roster), a team-wide calendar (one lane per teammate, all projects), and a
// per-project rollup. Same calm, light, mostly-grayscale house style as the
// tasks board — colour is rare and functional, never decorative.
...
function AdminPageInner() {
  ...
  // Client-side UX gate. Real authorization is enforced server-side by
  // /api/teams/[id]/command via verifyTeamAccess — non-members get a 403 and
  // the payload carries its own `role`. Keying the fetch off the UX gate just
  // avoids a doomed request for non-owners.
  const isOwnerUX = currentTeam?.role === 'owner';

  const { data, error, mutate } = useSWR<CommandResponse>(
    teamId && isOwnerUX ? `/api/teams/${teamId}/command` : null,
    fetcher,
  ...
  const loading = teamId != null && isOwnerUX && data === undefined && !error;
  ...
  if (teamLoading || !currentTeam) return null;
  if (currentTeam.role !== 'owner') {
    return (
      <div className="v2-tasks-locked">
        <div className="glass-card is-static is-roomy">
          <span className="recgon-label v2-locked-eye">{tTasks('ownerLock.eyebrow')}</span>
          <h2 className="v2-locked-title">{tTasks('ownerLock.title')}</h2>
          <p className="v2-locked-body">{tTasks('ownerLock.body')}</p>
        </div>
        <style>{`
          .v2-tasks-locked {
            padding: 20px 0 40px;
