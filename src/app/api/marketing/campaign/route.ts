...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (await isRateLimited(`campaign:team:${teamId}`, GENERATE_LIMIT)) {
      return NextResponse.json({ error: 'Your team has hit the campaign planning rate limit. Please wait a moment.' }, { status: 429 });
    }
