...
5. `rankMatches()` + `planTaskSchedule()` score every teammate × calendar slot; the best assignment writes via `assignTask()` and `setTaskSchedule()` (`src/lib/recgon/storage.ts`).
   - Phase 3 / Plan 06 refusal + deferral overlay: each task goes through `routeTaskOrTriage()` in `dispatcher.ts` before assignment. `hasMinimumFit()` (match.ts) demands at least one FIT signal (skillOverlap | fitForKind | interestNudge) ≥ `SIGNAL_FLOOR=0.15` — availability + load are explicitly NOT FIT signals (user rule 2026-05-15: never assign by availability alone). Four outcomes: assign / triage `no_clear_fit` / defer (scheduledDate moves forward via `deferTaskScheduledDate()`) / triage `no_capacity_in_window` / triage `no_capacity_high_priority` (priority ≥ `HIGH_PRIORITY_THRESHOLD=3` bypasses deferral). `findEarliestCapacityWindow()` scans up to `DEFER_LOOKAHEAD_WEEKS=4` for an opening. The grounded Why-you LLM returning null (Plan 03-05) is a 5th refusal path → triage `no_grounded_reason`. Refusal state lives on the new `agent_tasks.triage_note text` column (additive nullable; partial index for the upcoming TASKS-page triage view); deferral state lives on `schedule_note`. Storage helpers: `markTaskForTriage`, `deferTaskScheduledDate`, `clearTriageNote`. `DispatchResult` exposes `triaged` + `deferred` counters.
   - Phase 4 / Plan 01 personalized task framing: after a successful `assignTask`, the dispatcher fire-and-forget enqueues a `task_reframe` LLM job (`enqueueReframeJob` helper in `dispatcher.ts`, shared by `runDispatch` + `dispatchTask`). The worker (`runTaskReframe` in `src/lib/llm/workers.ts`) drains it, calls `runReframe()` in `src/lib/recgon/reframe.ts` (pure, adapter-injected `chatViaProviders` at temperature=0) with a bounded-tone prompt (FRAME-06 whitelisted moves: `fit_acknowledgement` / `start_location` / `recent_state_link`; FRAME-07 grounding validator rejects sentences citing skills not in `declaredSkills`), and writes the result to two new additive columns on `agent_tasks`: `personalized_description text null` + `personalized_description_for_user_id text null` (camelCase mirror: `AgentTask.personalizedDescription` + `AgentTask.personalizedDescriptionForUserId`). Worker fails-soft (`{skipped:true, reason:'columns_missing'}`) when the columns are absent (unmigrated env), and skips with `reason:'reassigned'` when the assignee changed between enqueue and run (race shield). FRAME-05 follow-up (2026-05-21): the reframe worker is now the SOLE owner of the assignment email — dispatcher no longer calls `notifyTeammateAssigned` directly. The worker reloads fresh task state via `getTask`/`getTeammate`/`getTeam` after writing `personalized_description` and sends the email with the personalized text (or original on `columns_missing` / `thin_profile` / `reframe_failed_all_retries` paths). Reassignment-race path sends NO email (the reframe job enqueued by `reassignTask` for the new assignee will send theirs). Pre-LLM thin-profile guard skips the LLM entirely when assignee has zero declared signals + no recent state (saves cost; grounding would reject anyway). Structured telemetry at every terminal outcome (`reframe_success` / `reframe_skipped` / `reframe_rejected` / `reframe_failed_all_retries` / `reframe_email_sent` / `reframe_email_send_failed`). NFKC normalization on tone-validator input catches full-width / compatibility-form bypasses; cross-script confusables (Cyrillic 'а' etc.) are documented out of scope.
   - Phase 4 / Plan 02 viewer-discriminated read: the GET route at `src/app/api/recgon/tasks/[id]/route.ts` computes `shouldServePersonalized = isAssignee && personalizedDescription non-empty && personalizedDescriptionForUserId === session.user.id` BEFORE building the response payload, then explicitly destructures `personalizedDescription`, `personalizedDescriptionForUserId`, `description`, and `assignmentReasoning` OUT of the task and overwrites `description` with the server-selected text — the raw personalized columns NEVER cross the API boundary (T-04-02-01 mitigation; destructure+overwrite, NOT spread). The userId-match check at the read boundary is the FRAME-04 safety net for the cron-cycle race window before Plan 04-03 sweeps the column on reassignment. `notifyTeammateAssigned` in `src/lib/notifications.ts` applies the same gate (`personalizedAvailable` + `escapeHtml` on the description body — T-04-02-03 defense in depth) and uses `teammate.userId` (NOT `task.assignedTo`, which is a teammateId) for the match. The client (`TaskDetailPanel.tsx`) reads `task.description` directly — no client-side branching; the API is the single source of truth.
