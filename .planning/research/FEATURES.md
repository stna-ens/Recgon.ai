# Feature Landscape — AI Product Manager v2

**Domain:** AI Product Manager / LLM-augmented task dispatcher for small dev teams
**Milestone:** Recgon v2 — Smarter dispatcher (live code signal + teammate profile + LLM judgment + personalized framing)
...
These are non-negotiable for v2 to feel like a "real PM" rather than a routing function. Missing any of these = users leave.
...
| TS-7 | **Self-profile UI** — teammates can declare skills, strengths, interests, capacity (and edit anytime) | M | New route under `teams/[id]/` | **Missing** — explicit v2 active requirement. |
| TS-8 | **Graceful fallback when LLM unavailable** — math winner ships if Gemini+Claude both down | S | Circuit breaker, provider chain | **Already exists** (circuit breaker fail-open). v2 requirement explicit. |
...
| TS-10 | **Backwards compatibility** — old tasks/teammates keep working through v2 rollout | S | Additive schema only | Discipline, not feature work. Migration plan required. |
| TS-11 | **Notification on assignment** — assignee learns about new task without polling the dashboard | S | `notifications.ts` + Resend email | **Already exists** (`notifyTeammateAssigned`). Verify it includes personalized framing in v2. |
...
| DIF-6 | **Stretch / cross-skill task flagging** — surface "you've never done this; could be a learning task" so owners can intentionally route growth assignments | M | DIF-3 (LLM sees skill gap), fitProfile.skillStats | **Cheaper**: EMA already tracks per-skill quality. Asking the LLM judgment overlay to flag stretch is a prompt extension, not a new pipeline. Not in v2 active list — viable v2.5 / v3. |
| DIF-7 | **Soft-factor variety enforcement** — if assignee did 3 backend tasks this week, prefer a frontend candidate for next backend task (unless skill gap is too wide) | M | DIF-3 LLM judgment, recent task history | **Cheaper**: recent assignments already in `agent_tasks`. v2 LLM overlay implicitly does this if prompted to consider "recent load mix." Not separate feature — fold into DIF-3 prompt. |
| DIF-8 | **Assignment audit log with both math + LLM reasoning** — every decision is reproducible and inspectable | S | DIF-3 | **Cheaper**: add columns to `agent_tasks` (`mathBreakdown` jsonb, `llmReasoning` text). v2 active requirement. |
| DIF-9 | **Three-source skill model convergence** — self-declared + GitHub-inferred + EMA-from-history, with the model showing which signal contributed what | M | TS-7 + DIF-2 + existing fitLearning | **Cheaper**: all three sources are already designed; needs unification layer + UI to display "your top skill `react` came from your commits + 3 completed tasks." v2 active requirement. |
...
| AF-8 | **Slack / calendar push notifications for delivery in v2** | Deferred until v2 ships. Adds integration surface that doesn't move the core loop. | In-app + Resend email only. Slack is a v3 candidate. |
| AF-9 | **Mobile-native dispatcher UI** | Mobile redirects to landing already; dispatcher is desktop-only. Mobile adds layout cost without core value. | Desktop only for v2. |
...
| AF-11 | **Vector store / new database for embeddings** | Constraint per PROJECT.md: Supabase Postgres is system of record; no new infra in v2 unless justified. | Postgres `pgvector` extension (if needed) or plain text+LLM retrieval over brain entries. Likely not needed for v2. |
...
## MVP Recommendation (v2 Phase Ordering)
...
- [Collaborating with AI Agents — arXiv 2503.18238](https://arxiv.org/html/2503.18238v2)
