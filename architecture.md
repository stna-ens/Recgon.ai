...
- `lib/llm/workers.ts` — per-kind workers. Wired: `idea_analysis`, `codebase_analysis`, `competitor_analysis`. Local-path (non-GitHub) codebase projects can't be queued — the worker runs in a separate function with no access to the caller's fs; those still surface inline errors.
...
| `lib/recgon/profileMerge.ts` | Phase 1 / PROFILE-04 / Plan 01-02. Pure 4-arg merge `profileMerge(teammate, profile, inferred=null, ema)` returning `Teammate & { interests: string[] }`: field-level fallback (D-06 self wins when filled; owner row fills blanks), strengths fold into the merged skills array (D-02 — `match.ts`'s existing Jaccard picks them up for free), interests carried as an additive field (D-03) for `match.ts` to read, EMA passthrough on `fitProfile`. Null profile → owner unchanged + `interests: []` (D-08). No Supabase, no LLM — trivially unit-testable. `inferred: null` is literal-type-locked in Phase 1 so Phase 2 will produce a TS error at every call site until they pass `InferredSkills`. |
| `lib/recgon/profileStorage.ts` | Phase 1 / PROFILE-03 / Plan 01-03. CRUD against the `teammate_profiles` table: `getProfile(teamId, userId)` (maybeSingle → nullable), `listProfiles(teamId)` (batch read for dispatcher wiring in Plan 04), `upsertProfile(input)` (onConflict `team_id,user_id` — single row per teammate per team), `mapTeammateProfile(row)` snake_case → camelCase mapper. Stores raw user text alongside canonical-filtered tags (D-14) plus `normalization_pending` (Pitfall 7 LLM-degraded marker). Service-role only — never imported by client components. |
