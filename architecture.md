...
| Route | Method | Auth | Notes |
|-------|--------|------|-------|
| `/api/inbox` | GET | Session | All non-terminal tasks assigned to teammate rows owned by the current user, decorated with team name. Each task includes `verification_status` + `verification_evidence` so the inbox can render Recgon's check state and inline proof requests. Returns `{ tasks, counts: { open, awaitingReview } }`. **Self-heals** stuck tasks: any task in `auto_running` with no `stage` and no `verdict` triggers a fire-and-forget `runTaskVerification` call (idempotent — the worker bails if the task already finalized). Recovers tasks left over from before the inline kick-off existed and tasks whose worker was killed mid-flight. **Phase 3.6 / Plan 04:** projection now includes `scheduled_until_date`, `overdue_tier`, and `last_overdue_action_at` so the `/tasks` UI can render the OverdueChip + overdue filter without a second fetch. Falls back gracefully via tiered `select` strings (reschedule cols + overdue cols → reschedule cols only → legacy) so environments that haven't applied the Plan 01 migration keep returning tasks. |
...
| `/api/cron/overdue-sweep` | GET/POST | `CRON_SECRET` | Phase 3.6. Vercel cron (`0 9 * * *`, daily 09:00 UTC). Calls `sweepOverdueTeams()` in `lib/recgon/storage.ts`. **Plan 03 (landed):** real implementation — iterates teams with `overdue_pressure_enabled=true`, loads active-status tasks + teammates + owner per team, calls `decideOverdueAction()`, then delegates to `executeOverdueAction()` in `lib/recgon/overdueRunner.ts` for emails + storage writes. Returns `{ teamsProcessed, actionsByKind: { nudge_teammate, escalate_to_owner, auto_reschedule, none } }`. Idempotent: the 24h cool-down on `last_overdue_action_at` short-circuits duplicate emails when the route is called twice in the same window. Skipped auth in local dev. |
| `/api/teams/[id]/tasks/[taskId]/snooze` | POST | Session, owner-only | Phase 3.6 / Plan 03. Body `{ days: number }` (integer 1–30). Pushes `scheduled_date` (and `scheduled_until_date` if multi-day) forward by N calendar days, resets `overdue_tier=0`, clears `last_overdue_action_at`, writes a `snoozed` row to `teammate_event_log`. 403 for non-owners. 400 for invalid `days`. 404 if task not found / wrong team. Backed by `snoozeTask()` in `lib/recgon/storage.ts`. **Plan 04 addition:** on success, also emits `logOverdueCounter('snoozed', { teamId, taskId })` so the structured `overdue_counter` log line lands alongside the four cron-side counters. |
...
| `teams` | `id`, `name`, `slug` (unique), `created_by`, `overdue_pressure_enabled boolean default true` (Phase 3.6 / Plan 01 — per-team kill switch for overdue-sweep cron). |
...
| `agent_tasks` | First-class tasks. `kind` enum: `next_step`/`dev_prompt`/`marketing`/`analytics`/`research`/`custom`. `source` enum: `brain`/`user`/`teammate`/`schedule`. `source_ref.dedupKey` provides idempotency for brain/scheduled mints (partial unique index). Status flow: `unassigned → assigned → in_progress → awaiting_review → completed`. `job_id` links to `llm_jobs` for AI runs. **Verification columns (`20260428_task_verification.sql`):** `proof` (jsonb), `verification_status` enum (`none`/`auto_running`/`auto_passed`/`auto_inconclusive`/`proof_requested`/`proof_evaluating`/`passed`/`failed`/`owner_override`), `verification_evidence` (jsonb: commit shas, metric deltas, verdict text, confidence, iterations), `verified_at`, `verified_by`. **Phase 3.6 / Plan 01 overdue-pressure columns (`20260519_overdue_pressure_schema.sql`):** `overdue_tier int default 0` (last tier acted on: 0=none, 1=nudged, 2=escalated, 3=auto_rescheduled), `last_overdue_action_at timestamptz` (24h cool-down timestamp). Overdue is DERIVED (`scheduled_date < today AND status IN active_set`), not a new status enum value. Partial index `idx_agent_tasks_overdue_sweep` on `(team_id, scheduled_date)` covers the sweep cron's hot path. |
...
| `lib/recgon/storage.ts` | CRUD + view query: `createTeammate`, `listTeammatesWithStats` (joins `team_members.role` → `teamRole` per teammate), `updateTeammate`, `retireTeammate`, `createTask`, `listTasks`, `assignTask`, `reassignTask`, `updateTaskStatus`, `setTaskSchedule` (schedule-only update — touches `scheduled_date`/`schedule_note`/`deadline` without mutating status or assignment), `loadHoursByDateForTeammate` / `loadHoursByDateForUser` (sum estimated hours per scheduled_date across active tasks; cross-team variant for humans), `deleteTask` (hard-deletes `agent_tasks` row + writes an `agent_task_tombstones` entry for `source='brain'` rows so the dedupKey isn't re-minted), `listTombstonedDedupKeys`, `setTaskProof`, `setTaskVerification`, `upsertRating`, `getRecgonState`, `saveBrainSnapshot`, `appendAssignmentLog`, `logEvent`. **Phase 3.6 / Plan 03 (landed):** `sweepOverdueTeams(today?)` — real iterator called by `/api/cron/overdue-sweep`. Loads teams with `overdue_pressure_enabled=true`, active-status tasks per team (`assigned/accepted/in_progress/awaiting_review`), teammates-with-stats, and owner (`teams.created_by` → `users.email`). Calls pure `decideOverdueAction()` per task, delegates non-`none` outcomes to `executeOverdueAction()` in `overdueRunner.ts`. Returns `{ teamsProcessed, actionsByKind }`. `markOverdueAction(taskId, tier, event, context?)` — atomic-ish double write: bumps `agent_tasks.overdue_tier` + `last_overdue_action_at=NOW` and inserts a `teammate_event_log` row (tier→event mapping: 1↔nudged, 2↔escalated, 3↔auto_rescheduled). `snoozeTask(taskId, days)` — owner-initiated escape hatch: shifts `scheduled_date` (and `scheduled_until_date` if set) forward by N days, resets `overdue_tier=0` + clears `last_overdue_action_at`, writes a `snoozed` event-log row. `logEvent` accepts `nudged`, `escalated`, `auto_rescheduled`, `snoozed` event types. |
| `lib/recgon/overdueRunner.ts` | Phase 3.6 / Plan 03 — side-effect dispatcher for `decideOverdueAction()`. `executeOverdueAction(task, action, teammate, owner, today?)` switches on `action.kind`: **tier 1 (nudge_teammate)** → `sendOverdueNudge` to the assignee (via `lib/email.ts`) then `markOverdueAction(task.id, 1, 'nudged')`; **tier 2 (escalate_to_owner)** → `sendOverdueEscalation` to the team owner then `markOverdueAction(task.id, 2, 'escalated')`; **tier 3 (auto_reschedule)** → `planTaskSchedule()` over a 21-day lookahead with cross-team load aggregation; on success, `setTaskSchedule()` with the original date preserved in `schedule_note` for audit, then `sendAutoReschedulNotice` to the assignee; on no-capacity, falls back to `reassignTask(taskId, null, 'recgon')` (handoff to dispatcher). Same `today` clock is used for both the policy and the email body so day-count drift is impossible mid-sweep. Email lookup hits `users.email` via the assigneeʼs `teammate.userId`; missing emails skip the send and log at debug level. No LLM calls. |
...
| `lib/email.ts` | Resend wrappers. `sendOtpEmail(email, code)` — registration verification HTML email (throws on Resend error so the auth route can surface failure). **Phase 3.6 / Plan 03:** `sendOverdueNudge({to, taskTitle, daysOverdue, taskUrl})`, `sendOverdueEscalation({to, assigneeName, taskTitle, daysOverdue, taskUrl})`, `sendAutoReschedulNotice({to, taskTitle, newDate, taskUrl})` — fire-and-forget plain-text emails for the overdue-pressure sweep. Copy comes from `prompts.ts:OVERDUE_*_TEMPLATE`. No `RESEND_API_KEY` → silent skip + debug log (keeps the sweep loop alive for the next task). |
...
| `lib/recgon/overduePolicy.ts` | Phase 3.6 / Plan 02 — **pure** overdue-policy decision function. `decideOverdueAction({ task, teamOverduePressureEnabled, today }) → OverdueAction` runs the locked-in decision tree (assigned-only, active-status-only, has-scheduled-date, feature-flag-respecting, then days-overdue → tier band → no-skip clamp at `task.overdueTier + 1` → 24h cool-down on `lastOverdueActionAt`). Bands: `1–2 days → nudge_teammate (tier 1)`, `3–6 days → escalate_to_owner (tier 2)`, `7+ days → auto_reschedule (tier 3)`. Exports `ACTIVE_OVERDUE_STATUSES = ['assigned','accepted','in_progress','awaiting_review']`, `daysOverdue(scheduledEndKey, today)`, `isOverdue(task, today)` (UI helper for Plan 04). Zero I/O / zero LLM / zero Supabase — source contains no `new Date()` / `Date.now()`, the cron passes today in. Covered by `src/__tests__/overduePolicy.test.ts` (41 cases, 100% line coverage). Plan 03 wires the side effects (Resend emails + storage writes) on top of this brain. |
| `lib/logger.ts` | Minimal structured logger with level threshold (`LOG_LEVEL` env, default `info` in prod / `debug` in dev). Exports `logger.{debug, info, warn, error}`. **Phase 3.6 / Plan 04:** `logOverdueCounter(kind, { teamId, taskId })` — fixed-shape `overdue_counter` info line. `kind` is one of `nudged` / `escalated` / `auto_rescheduled` / `snoozed`. Called from `overdueRunner.executeOverdueAction` after each successful tier-action and from the snooze API route on success. One line per action — downstream log aggregators bucket per team without parsing free-form messages. |
...
- Calendar (v2): `/v2/projects/[id]/tasks` is a **day-precision swim-lane week calendar** (`WeekCalendar.tsx`). Rows = teammates, columns = Mon–Sun. Single-day tasks render as cards in their `scheduledDate` cell; multi-day tasks (`scheduledDate` < `scheduledUntilDate`) render as one chip spanning across the cells they occupy. No hour-of-day anywhere (avoids cross-TZ ambiguity). Per-day capacity bar shows assigned hours vs the teammate's daily capacity (`capacityHours / workingHours.days.length`); a multi-day task contributes its hours evenly across each day in its range. Fetches via `/api/teams/[id]/calendar`. Toggle switches to the legacy list view (`list-view.tsx`). Components: `src/components/v2/calendar/`. **Phase 3.6 / Plan 04 — overdue surfacing:** `OverdueChip.tsx` (signature-pink outline / warning-amber outline / danger-red filled — owner view encodes tier, assignee view shows uniform pink) is wired through `EventChip` (chip rendered on every task card that satisfies `isOverdue(task, today)`) and the `WeekCalendar` threads `ownerView={isOwner}` through `SwimLane → EventChip` so the project Calendar tab shows tier-2/3 escalation color only to the team owner. `PersonalCalendar.tsx` keeps `ownerView` at the default `false` — cross-team aggregation has no single "owner". `TaskDetailPanel.tsx` renders the chip inline next to the SCHEDULED date and, when the viewer is owner AND the task is overdue, also renders the owner-only `SnoozeControl` (1d / 3d / 7d preset + custom 1–30 input, POSTs to `/api/teams/[id]/tasks/[taskId]/snooze`). Same `OverdueChip` component is re-used on the `/tasks` page cards (assignee view) alongside a new `overdue` filter pill that narrows the kanban to `isOverdue(task, today) === true` (empty state: `nothing slipping — you're on top of it`). No new design-system tokens introduced — chip uses existing `var(--signature)` / `var(--warning)` / `var(--danger)`.
