...
| `/api/cron/overdue-sweep` | GET/POST | `CRON_SECRET` | Phase 3.6. Vercel cron (`0 9 * * *`, daily 09:00 UTC). Calls `sweepOverdueTeams()` in `lib/recgon/storage.ts` — currently a stub returning `{ teamsProcessed: 0, actionsByKind: { nudge_teammate, escalate_to_owner, auto_reschedule, none } }`. **Plan 02 (landed):** `decideOverdueAction()` (pure policy) is now available in `lib/recgon/overduePolicy.ts` — the sweep wiring lands in Plan 03 along with Resend emails + storage writes for graduated overdue pressure (nudge → escalate → auto_reschedule). Skipped auth in local dev. |
